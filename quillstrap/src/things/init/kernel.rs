...
impl SetupThing for Kernel {
    ...
    fn build(&self, options: &crate::Options) -> color_eyre::eyre::Result<(), String> {
        ...
        run_shell_command("make ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- pinenote_defconfig", options.config.command_output).unwrap();
        
        run_shell_command(
            &format!("make -j{} ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu-", get_cores()),
            options.config.command_output,
        )
        .unwrap();

        let path_of_kernel_absolute = get_path_of_thing(&get_thing_by_name(self.name(), &options.things), options);
        run_shell_command(
            &format!("make -j{} ARCH=arm64 CROSS_COMPILE=aarch64-linux-gnu- modules_install INSTALL_MOD_PATH=\"{}initrd_base/\"", get_cores(), path_of_kernel_absolute),
            options.config.command_output,
        )
        .unwrap();
