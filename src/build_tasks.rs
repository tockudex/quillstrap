...
use common::{bind_mount, clean_dir, download_file, run_command};
...
pub fn rootfs(
    ...
    let base_rootfs_archive_path = format!(
        "{}/../{}/{}",
        &build_dir,
        &common::DOWNLOADS_DIR,
        &common::BASE_ROOTFS_FILE
    );
    let base_rootfs_path = format!(
        "{}/{}/{}",
        &build_dir,
        &common::ROOTFS_BUILD_DIR,
        &common::ROOTFS_BASE_DIR
    );
    ...
    if !fs::exists(&base_rootfs_archive_path)? {
        ...
        let latest_build = Command::new("bash")
            .args([
                "-c",
                &format!(
                    "curl -s {} | grep href | tail -n 3 | cut -c 10-25 | tail -n 1",
                    &common::BASE_ROOTFS_URL
                ),
            ])
            .output()?;
    ...
    run_command(
        "tar",
        &["-C", &base_rootfs_path, "-xvf", &base_rootfs_archive_path],
    )?;
    // Set up the chroot
    rootfs_setup_chroot(
        &rootfs_build_dir_path,
        &base_rootfs_path,
        &merged_rootfs_path,
        unrestricted_system,
    )?;
    ...
    rootfs_misc(&merged_rootfs_path, unrestricted_system)?;
    rootfs_tear_down_chroot(&merged_rootfs_path)?;
    // Compress the rootfs
    rootfs_compress_and_sign(&rootfs_build_dir_path, &merged_rootfs_path, &private_key)?;

    Ok(())
}

pub fn rootfs_setup_chroot(
    rootfs_build_dir_path: &str,
    base_rootfs_path: &str,
    merged_rootfs_path: &str,
    unrestricted_system: bool,
) -> Result<()> {
    let rootfs_overlay_repository_path =
        format!("{}/{}", &rootfs_build_dir_path, &common::ROOTFS_OVERLAY_DIR);
    ...
    Repository::clone_recurse(
        &common::ROOTFS_OVERLAY_REPO_URL,
        &rootfs_overlay_repository_path,
    )?;
    ...
    run_command(
        ...
        &[
            ...
            &format!(
                ...
                &common_rootfs_overlay_path,
                &specific_rootfs_overlay_path,
                &base_rootfs_path,
                &rootfs_cow_dir_path,
                &rootfs_work_dir_path
            ),
            &merged_rootfs_path,
        ],
    )?;
    ...
    Mount::builder()
        .fstype("proc")
        .mount("proc", &format!("{}/proc", &merged_rootfs_path))?;
    Mount::builder()
        .fstype("sysfs")
        .mount("sysfs", &format!("{}/sys", &merged_rootfs_path))?;
    Mount::builder()
        .fstype("tmpfs")
        .mount("tmpfs", &format!("{}/tmp", &merged_rootfs_path))?;
    Mount::builder()
        .fstype("tmpfs")
        .mount("tmpfs", &format!("{}/run", &merged_rootfs_path))?;
    Mount::builder()
        .fstype("devtmpfs")
        .mount("devtmpfs", &format!("{}/dev", &merged_rootfs_path))?;
    ...
    bind_mount("/etc/resolv.conf", &resolv_conf_path)
        .with_context(|| "Could not bind-mount resolv.conf")?;
...
pub fn rootfs_tear_down_chroot(merged_rootfs_path: &str) -> Result<()> {
    unmount(
        format!("{}/proc", &merged_rootfs_path),
        UnmountFlags::DETACH,
    )?;
    ...
    unmount(
        format!("{}/etc/resolv.conf", &merged_rootfs_path),
        UnmountFlags::DETACH,
    )?;
...
pub fn rootfs_manage_packages(merged_rootfs_path: &str, unrestricted_system: bool) -> Result<()> {
    ...
    if unrestricted_system {
        rootfs_run_chroot_command(
            &merged_rootfs_path,
            &["dnf", "--assumeyes", "install", "dropbear"],
        )?;
    }
...
pub fn rootfs_misc(merged_rootfs_path: &str, unrestricted_system: bool) -> Result<()> {
    if unrestricted_system {
        File::create(&format!("{}/.unrestricted", &merged_rootfs_path))?;
    }
    ...
    rootfs_run_chroot_command(
        ...
        &[
            "sed",
            "-i",
            "s/Fedora Linux 42 (Container Image)/Quill OS/g",
            "/etc/os-release",
        ],
    )?;
...
pub fn rootfs_compress_and_sign(
    rootfs_build_dir_path: &str,
    merged_rootfs_path: &str,
    private_key: &PKey<Private>,
) -> Result<()> {
    ...
    signing::sign_file(&private_key, &archive_path)?;
